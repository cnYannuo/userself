...
$(".imgfile").live("change", function () {
    ...
    for(var mg=0;mg<this.files.length;mg++){
        //判断图片格式是否正确
        $("#photo_num").html($(".lookimg").length+1);
        ...
        //判断图片是否过大，当前设置1MB
        var file = this.files[mg];//获取file文件对象
        ...
        previewName.innerHTML=this.files[mg].name;
        $(_prevdiv).append(previewName);
